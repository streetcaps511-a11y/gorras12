...
export const useClientesLogic = () => {
  ...
  const openModal = (mode = 'create', cliente = null) => {
    ...
    if (cliente && (mode === 'edit' || mode === 'view')) {
      ...
      setFormData({
        ...
        city: cliente.ciudad,
        department: dept?.id || '',
        stateProvince: cliente.estadoProvincia || '',
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === 'country') {
      setFormData(prev => ({ ...prev, country: value, department: '', city: '', stateProvince: '' }));
      setCiudades([]);
  ...
  const validateForm = () => {
    ...
    if (formData.country === 'Colombia') {
      ...
      if (!formData.stateProvince?.trim()) newErrors.stateProvince = 'Estado/Provincia es obligatorio';
  ...
  const handleSave = async () => {
    ...
    const apiClienteData = {
      ...
      departamento: deptObj?.name || formData.stateProvince || formData.department,
      estadoProvincia: formData.stateProvince || '',
      saldoFavor: '0',
